...
QUESTP <- function(Fun, #function to be fitted, needs to output a probability of seen
    ...
    while(!QUESTP.stop(state)) {
        ...
        if (verbose == 2) {
            ...
            cat(sprintf("stim= %5s repsonse=%s ", paste0(as.numeric(tail(state$stimuli,1)), collapse = ", "), 
...
#Gets the optimal stimulus to minimize entropy - useful to export for nextStim implementation
#' @rdname QUESTP
